...
for n = 1:length(yy)
    if onecolumn
        ...
        if n==4
            filename = 'SwitchingModesFanning_1'; 
            try
    ...
    [dd0, Hsw0] = GetMagnetosomeLine(ang_sw, y, e, d, N, yy(n), 1.0, 10); 
    [dd1, Hsw1] = GetMagnetosomeLine(ang_sw, y, e, d, N, yy(n), 1.1, 10); 
    [dd2, Hsw2] = GetMagnetosomeLine(ang_sw, y, e, d, N, yy(n), 1.2, 10); 
    [dd3, Hsw3] = GetMagnetosomeLine(ang_sw, y, e, d, N, yy(n), 1.3, 10); 
    [dd4, Hsw4] = GetMagnetosomeLine(ang_sw, y, e, d, N, yy(n), 1.4, 10); 
...
if onecolumn
    filename = 'SwitchingModesFanning_2'; 
else
    filename = 'SwitchingModesFanning'; 
end
try
    export_fig(sprintf('../output/png/%s.png', filename), '-m4'); 
    export_fig(sprintf('../output/pdf/%s.pdf', filename)); 
