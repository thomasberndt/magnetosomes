...
for n = 1:length(yy)
    ...
    [dd0, Hsw0] = GetMagnetosomeLine(vort_r, y, e, d, N, yy(n), 1.0, 10); 
    [dd1, Hsw1] = GetMagnetosomeLine(vort_r, y, e, d, N, yy(n), 1.1, 10); 
    [dd2, Hsw2] = GetMagnetosomeLine(vort_r, y, e, d, N, yy(n), 1.2, 10); 
    [dd3, Hsw3] = GetMagnetosomeLine(vort_r, y, e, d, N, yy(n), 1.3, 10); 
    [dd4, Hsw4] = GetMagnetosomeLine(vort_r, y, e, d, N, yy(n), 1.4, 10); 
    ...
    ylabel('Vorticity at remanence [arbitrary units]'); 
...
if onecolumn
    filename = 'RemanentModes_2'; 
else
    filename = 'RemanentModes'; 
end
try
    export_fig(sprintf('../output/png/%s.png', filename), '-m4'); 
    export_fig(sprintf('../output/pdf/%s.pdf', filename)); 
