...
ww = [w w(end:-1:1)];
good = ~isnan(Mr3); 
h1 = patch(ww(good), Mr3(good), [.75 .75 1], ...
        'EdgeColor', 'none');
hold on
...
ww = [w w(end:-1:1)];
good = ~isnan(Mr3); 
h2 = patch(ww(good), Mr3(good), [.75 1 .75], ...
        'EdgeColor', 'none');
...
set(gca,'FontSize',18);
xlabel('Mean grain size (width) [nm]'); 
ylabel('Normalized remanent Magnetization M_r/M_s [Am^2]'); 
hold off

%%

filename = 'TEM_morphology_Mr'; 
